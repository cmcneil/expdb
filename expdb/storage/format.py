...
TYPE_TO_EXTENSION = {}
TYPE_TO_SERIALIZER: Dict[Tuple[DataType, type],
                         Type['PayloadSerializer[TimecoursePayload]']] = {}
...
class PayloadSerializer(ABC, Generic[T]):
    """Abstract base class for serializing and deserializing timecourse payloads.

    This class defines the interface for serializing different types of timecourse data
    (e.g. numpy arrays, pandas DataFrames, MNE Raw objects) to and from files. Each
    concrete implementation handles a specific payload type and file format.

    The class uses a generic type parameter T (bound to TimecoursePayload) to ensure
    type safety when implementing concrete serializers.

    Attributes:
        extension (str): The file extension associated with this serializer's format.

    To implement a concrete serializer:
    1. Subclass PayloadSerializer with a specific payload type
    2. Set the extension class variable
    3. Implement _write_to_file() and _read_from_file() methods
    4. Use @register method to register supported DataTypes

    By implementing this interface, the framework will automatically handle data
    retrieval and storage for Timecourse objects of the given type.
    """
...
class VideoPayloadSerializer(PayloadSerializer[np.ndarray]):
    ...
    def _write_to_file(self, payload: np.ndarray, fname: str):
        ...
        if len(payload.shape) != 4:
            raise ValueError(f"Video payload must have 4 dimensions but got {
                             len(payload.shape)}")
        if payload.dtype != np.uint8:
            raise ValueError(
                f"Video payload must have dtype uint8 but got {payload.dtype}")

        # Get video dimensions
        n_frames, height, width, channels = payload.shape
        if channels != 3:
            raise ValueError(
                f"Video payload must have 3 channels but got {channels}")
...
class AudioPayloadSerializer(PayloadSerializer[np.ndarray]):
    ...
    def _write_to_file(self, payload: np.ndarray, fname: str):
        ...
        if len(payload.shape) != 2:
            raise ValueError(f"Audio payload must have 2 dimensions but got {
                             len(payload.shape)}")
        if payload.dtype != np.float32:
            raise ValueError(
                f"Audio payload must have dtype float32 but got {payload.dtype}")

        # Get audio properties
        samples, channels = payload.shape
        if channels > 2:
            raise ValueError(
                f"Audio payload must have 1 or 2 channels but got {channels}")
...
class FMRIPayloadSerializer(PayloadSerializer[np.ndarray]):
    ...
    def _write_to_file(self, payload: np.ndarray, fname: str):
        ...
        if len(payload.shape) < 3:
            raise ValueError(f"FMRI payload must have at least 3 dimensions but got {
                             len(payload.shape)}")
